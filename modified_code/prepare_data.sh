...
NUCLE_TAR=$DATA_DIR/../data/orign_data/release3.2.tar
LANG8V2=$DATA_DIR/data/orign_data/lang-8-20111007-L1-v2.dat
...
python $M2_SCRIPTS/sort_m2.py  -i $NUCLE_DIR/data/conll14st-preprocessed.m2 \
                     -o $DATA_DIR/tmp/nucle.sort.m2 \
                     -m 1 --output-remaining-lines
python $M2_SCRIPTS/get_num_lines.py    -i $DATA_DIR/tmp/nucle.sort.m2 \
...
python $M2_SCRIPTS/convert_m2_to_parallel.py   $DATA_DIR/nucle-train/nucle-train.m2 \
                                     $DATA_DIR/nucle-train/nucle-train.tok.src \
                                     $DATA_DIR/nucle-train/nucle-train.tok.trg
python $M2_SCRIPTS/convert_m2_to_parallel.py   $DATA_DIR/nucle-dev/nucle-dev.m2 \
...
mkdir -p $DATA_DIR/tmp
python $LANG8_SCRIPTS/extract.py -i $LANG8V2 -o $DATA_DIR/tmp/ -l2 $L2
...
python $LANG8_SCRIPTS/langidfilter.py $DATA_DIR/tmp/lang-8.$L2.cleanedup > $DATA_DIR/tmp/lang-8.$L2.extracted
rm $DATA_DIR/tmp/lang-8.$L2.cleanedup
python $LANG8_SCRIPTS/get_parallel.py -i $DATA_DIR/tmp/lang-8.$L2.extracted -o lang-8 -d $DATA_DIR/tmp/lang-8/

for EXT in src trg; do
    cat $DATA_DIR/tmp/lang-8/lang-8.$EXT | $REPLACE_UNICODE | $REMOVE_NON_PRINT | sed  's/\\"/\"/g' | sed 's/\\t/ /g' | $NORMALIZE_PUNCT |  python $TOKENIZER  > $DATA_DIR/lang-8/lang-8.tok.$EXT
